...
public class HashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        V result = null;
        double currentRatio = (double) (size + 1) / (double) (table.length);
        if (currentRatio >  MAX_LOAD_FACTOR) {
        ...
        if (table[hashnum] == null) {
            ...
            } while (check.getNext() != null); // end of do-while loop
            if (check.getKey().equals(key)) {
                result = check.getValue();
                check.setValue(value);
                return result;
            }
            MapEntry<K, V> addEle = new MapEntry(key, value, table[hashnum]);
            table[hashnum] = addEle;
            size++;
            return null;
        }
    }
    ...
    public void resizeBackingTable(int length) {
        if (length <= 0 || length < size) {
            throw new IllegalArgumentException("Length value is illegal.");
        }
        int currsize = this.size;
        MapEntry<K, V> currPair;
        this.size = 0;
        MapEntry<K, V>[] currentTable = this.table;
        table = (MapEntry<K, V>[]) new MapEntry[length];

        for (MapEntry<K, V> pair: currentTable) {
            // pair is the beginning of each linked list at
            // the mash number position
            this.size = 0;
            if (pair != null) {
                // relocate each key, value pair into the new right position
                put(pair.getKey(), pair.getValue());
                currPair = pair;
                while (currPair.getNext() != null) {
                    // relocate all node (key, value pair) in a linked list
                    put(currPair.getKey(), currPair.getValue());
                    currPair = currPair.getNext();
                }
                // relocate the last node of a linked list
                put(currPair.getKey(), currPair.getValue());
            }
        }
        this.size = currsize;
    ...
    public V remove(K key) {
        if (key == null) {
            throw new java.util.NoSuchElementException("Key is null");
        }
        MapEntry<K, V> curr;
        int count = 0; // the index of the node in the linkedlist
        int hashnum = Math.abs(key.hashCode() % table.length);
        if (table[hashnum] == null) {
            throw new java.util.NoSuchElementException(
                    "Key does not exist.");
        } else {
            // check begins at the first node of the linked list
            MapEntry<K, V> check = table[hashnum];
            do {
                if (check.getKey().equals(key)) {
                    // the first node is the one we want to remove
                    if (count == 0) {
                        if (check.getNext() == null) {
                            table[hashnum] = null;
                        } else {
                        ...
                        // curr ends at the previous node of check
                        if (check.getNext() == null) {
                            curr.setNext(null);
                        } else {
            ...
            // check the last node in the linkedlist
            if (check.getKey().equals(key)) {
                // if this is the only node in the linkedlist
                if (count == 0) {
                    if (check.getNext() == null) {
                        table[hashnum] = null;
                    } else {
    ...
    public V get(K key) {
        ...
        if (table[hashnum] == null) {
            ...
            // check the last node in the array
            if (check.getKey().equals(key)) {
                return check.getValue();
            }
        }
